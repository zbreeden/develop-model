...
import argparse, csv, json, random, uuid, math, re, shutil
...
def main():
    ...
    ap.add_argument("--mirror-data", action="store_true", help="Also copy the generated file into data/internal/ (searched from spec location upwards or CWD).")
    args = ap.parse_args()
    ...
    # If the spec is inside a FourTwentyGenesis repo, prefer writing into its data/internal/ dir
    try:
        # walk parents to find a directory named 'FourTwentyGenesis'
        repo_root = None
        for p in [spec_path.resolve().parent] + list(spec_path.resolve().parents):
            if p.name == "FourTwentyGenesis":
                repo_root = p
                break
        if repo_root is not None:
            # if output is a relative path (not absolute), replace it with repo data/internal/<basename>
            out_path = Path(output)
            if not out_path.is_absolute():
                output = str(repo_root / "data" / "internal" / out_path.name)
    except Exception:
        # on any failure fall back to spec-defined output
        pass
    ...
    # Optionally mirror to data/internal/
    mirror_flag = spec.get("mirror_to_data_internal", False) or args.mirror_data
    if mirror_flag:
        # If spec provides a string path, use it as the target directory
        if isinstance(mirror_flag, str) and mirror_flag.strip():
            target_dir = Path(mirror_flag).expanduser()
        else:
            # Search upward from the spec path for a 'data' directory; if found, use its parent as repo root
            target_dir = None
            for p in [spec_path.resolve().parent] + list(spec_path.resolve().parents):
                candidate = p / "data" / "internal"
                if (p / "data").exists():
                    target_dir = candidate
                    break
            if target_dir is None:
                # Fallback to CWD/data/internal
                target_dir = Path.cwd() / "data" / "internal"

        try:
            target_dir.mkdir(parents=True, exist_ok=True)
            dest = target_dir / Path(output).name
            shutil.copy2(Path(output), dest)
            print(f"Also copied output to {dest}")
        except Exception as e:
            print(f"Warning: failed to mirror output to data/internal/: {e}")
